...
export default function ReferClientModal({
  ...
  const submit = async () => {
    if (!form.client_name || !form.phone || !form.location || !form.property_name) {
      Alert.alert("Missing fields", "Please fill name, phone, location and property.");
      return;
    }
    setBusy(true);
    try {
      await api("/referrals", { method: "POST", body: JSON.stringify({ ...form, bhk, ownership }) });
      reset();
      onSubmitted?.();
      onClose();
      Alert.alert("Submitted", "Referral pushed to designers.");
    } catch (e: any) {
